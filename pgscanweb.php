<?php
    ...
    function updateMeta(&$site, &$head, &$id)
    {
        ...
        if(!empty($title) OR isset($tete['description']) OR isset($tete['keywords'])){
            $DB->query('UPDATE liste SET'
                        .(!empty($title) ? ' title=\''.substr($title, 0, 250).'\'' : '')
                        .((isset($tete['description']) AND !empty($title) OR !empty($title) AND isset($tete['keywords'])) ? ', ' : '')
                        .(isset($tete['description']) ? ' description=\''.substr($tete['description'], 0, 300).'\'' : '')
                        .((isset($tete['description']) AND isset($tete['keywords'])) ? ', ' : '')
                        .(isset($tete['keywords']) ? ' keywords=\''.substr($tete['keywords'], 0, 300).'\'' : '').' 
                        WHERE id='.$id);
        }
    }
    ...
    {
        $urls=$DB->queryArray('SELECT url, id FROM liste WHERE visite=0 AND en_visite=0 ORDER BY id LIMIT 500');
        foreach($urls as $uri)
